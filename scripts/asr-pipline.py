...
from asr.read_config import *
from asr.run_msas import *
from asr.fscores import *
from asr.asr_bayes import *
from asr.aws_tools import *
from asr.struct_analysis import *
from asr.index_mutations import *
from asr.log import *
...
# Will stop the script upon reaching, but not completing, this step.
...
if enable_aws is None:
    enable_aws = False
else:
    enable_aws = True

S3_BUCKET = None  # the name of a bucket in S3
# the base of a key string for this job, presumably this string will be
# the job ID.
S3_KEYBASE = None
if enable_aws:
...
if enable_aws:
    aws_update_status("Sequences and Configuration are OK",
                      S3_BUCKET, S3_KEYBASE)
...
if jump <= 1.1 and stop > 1.1:
    ...
    if enable_aws:
        aws_update_status("Checking the Aligned Sequences",
                          S3_BUCKET, S3_KEYBASE)
        push_database_to_s3(dbpath, S3_BUCKET, S3_KEYBASE)
...
if jump <= 1.11 and stop > 1.11:
    ...
    if enable_aws:
        aws_update_status(
            "Building a Site Map Between Different Alignments", S3_BUCKET, S3_KEYBASE)
        push_database_to_s3(dbpath, S3_BUCKET, S3_KEYBASE)
...
if not ap.getOptionalToggle("--skip_zorro"):
    ...
    if jump <= 2.41 and stop > 2.41:
        ...
        if enable_aws:
            aws_update_status("Running the ZORRO Analysis",
                              S3_BUCKET, S3_KEYBASE)
            push_database_to_s3(dbpath, S3_BUCKET, S3_KEYBASE)
...
if jump <= 2.7 and stop > 2.7:
    ...
    if enable_aws:
        aws_update_status("Converting all FASTA to PHYLIP",
                          S3_BUCKET, S3_KEYBASE)
        push_database_to_s3(dbpath, S3_BUCKET, S3_KEYBASE)
...
if jump <= 3 and stop > 3:
    ...
    if enable_aws:
        aws_update_status(
            "Finding ML Phylogenies with RAxML (this can take a while)", S3_BUCKET, S3_KEYBASE)
        push_database_to_s3(dbpath, S3_BUCKET, S3_KEYBASE)
...
if jump <= 4 and stop > 4:
    ...
    if enable_aws:
        aws_update_status(
            "Calculating Phylogenetic Branch Support", S3_BUCKET, S3_KEYBASE)
        push_database_to_s3(dbpath, S3_BUCKET, S3_KEYBASE)
...
if jump <= 4.2 and stop > 4.2:
    ...
    if enable_aws:
        aws_update_status("Computing Distances Between Trees",
                          S3_BUCKET, S3_KEYBASE)
        push_database_to_s3(dbpath, S3_BUCKET, S3_KEYBASE)
...
if jump <= 5 and stop > 5:
    ...
    if enable_aws:
        aws_update_status("Reconstructing Ancestral Sequences",
                          S3_BUCKET, S3_KEYBASE)
        push_database_to_s3(dbpath, S3_BUCKET, S3_KEYBASE)
...
if jump <= 5.11 and stop > 5.11:
    if enable_aws:
        aws_update_status("Matching Ancestors Across Models",
                          S3_BUCKET, S3_KEYBASE)
        push_database_to_s3(dbpath, S3_BUCKET, S3_KEYBASE)
...
if jump <= 5.2 and stop > 5.3:
    ...
    if enable_aws:
        aws_update_status("Extracting Relevant Ancestors",
                          S3_BUCKET, S3_KEYBASE)
        push_database_to_s3(dbpath, S3_BUCKET, S3_KEYBASE)
...
if jump <= 6 and stop > 6:
    if "compareanc" in ap.params:
        ...
        if enable_aws:
            aws_update_status(
                "Calculating Df Ranks for Functional Loci", S3_BUCKET, S3_KEYBASE)
            push_database_to_s3(dbpath, S3_BUCKET, S3_KEYBASE)
...
if x:
    if jump <= 6.5 and stop > 6.5:
        ...
        if enable_aws:
            aws_update_status("Calculating dN/dS Test Metrics",
                              S3_BUCKET, S3_KEYBASE)
            push_database_to_s3(dbpath, S3_BUCKET, S3_KEYBASE)
    ...
    if jump <= 6.8 and stop > 6.8:
        ...
        if enable_aws:
            aws_update_status(
                "Comparing Df Ranks to dN/dS Test Scores", S3_BUCKET, S3_KEYBASE)
            push_database_to_s3(dbpath, S3_BUCKET, S3_KEYBASE)
    ...
    write_log(
        con, "Codon sequences were not given by the user; I will skip dN/dS analysis.")
...
if jump <= 7 and stop > 7:
    ...
    if enable_aws:
        aws_update_status(
            "Almost Done: Cleaning Residual Files", S3_BUCKET, S3_KEYBASE)
        push_database_to_s3(dbpath, S3_BUCKET, S3_KEYBASE)
