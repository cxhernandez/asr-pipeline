...
def get_header(head="", urlpre=""):
    ...
    h += "<html>\n"
    h += "<head>\n"
    h += "<title>" + ap.params["project_title"] + \
        " Ancestral Library</title>\n"
...
def write_index(con):
    ...
    fout.write(
        "<p>This library was auto-generated by PhyloBot, using a collection of curated sequences.</p>\n")
    fout.write("<p><strong>Download Sequences:</strong> <a href=\"../" +
               ap.params["ergseqpath"] + "\">FASTA format, not aligned</a></p>\n")
    ...
    if "mltree_pdfpath" in ap.params:
        fout.write("<table>\n")
        fout.write("<tr><td>")
        fout.write("<a href='" + ap.params["mltree_pdfpath"] + "<img src='" + ap.params[
                   "mltree_pdfpath"] + "', height=200px></a>")
        fout.write("</td><td>")
        fout.write("<p></p>")
        fout.write("</td></tr>")
        fout.write("<\table>\n")
...
def write_alignments(con):
    ...
    for d in get_alignment_method_names(con):
        ...
        out += "<td align='center'><a href='../" + fpath + \
            "'>fasta</a> | <a href='../" + ppath + "'>phylip</a></td>"
        out += "</tr>\n"
    out += "</table>\n"
...
def write_treesancs(con):
    ...
    for d in get_alignment_method_names(con):
        ...
        for model in get_phylo_modelnames(con):
            ...
            out += "<td>" + model + "</td>"
            out += "<td align='right'>" + \
                model_data[model][0].__str__() + "</td>"
            out += "<td align='center'>" + \
                model_data[model][1].__str__() + "</td>"
            out += "<td align='center'>" + "%.3f" % tlength + "</td>"
            out += "<td align='center'><a href='../" + tpath + "'>newick</a></td>"
            out += "<td align='center'><a href='tree." + \
                d + "." + model + ".html'>view</a></td>"
            out += "</tr>\n"
...
def write_anctree(d, model):
    ...
    out += "<h2>Cladogram of Ancestors, Alignment: " + \
        get_msa_nickname(d) + ", Model: " + model + "</h2>\n"
...
def pp_distro_to_html(data):
    ...
    out += "</tr>\n"
    for site in data:
        if notdata[site][0].__contains__("-"):
...
def write_ppdistro_plot(data):
    ...
    keys = ["[0.0,0.1)", "[0.1,0.2)", "[0.2,0.3)", "[0.3,0.4)", "[0.4,0.5)",
        ...
        for model in get_phylo_modelnames(con):
            ...
            for f in os.listdir(ancdir):
                if f.__contains__(".dat"):
                    ...
                    out += "<h2>Ancestral Node " + nodenum + ", Alignment: " + \
                        get_msa_nickname(d) + ", Model: " + model + "</h2>\n"
                    # out += "<hr>\n"
                    out += "<div>\n"
                    ...
                    out += "<p>" + \
                        get_ml_sequence_from_file(ancdir + "/" + f) + "</p>\n"
                    ...
                    out += " each site in Node " + nodenum + \
                        ". Residues were sampled with a probability equal to the model-based  "
                    ...
                    out += "<p>Download 100 Random Sampled Sequences: <a href=\"../../" + \
                        ancdir + "/BAYES_SAMPLES/bayes." + f + "\">fasta</a></p>"
                    out += "</div>"
                    ...
                    out += "<p>The following table shows the full posterior probability distribution at each site in the sequence for Node " + \
                        nodenum + ". Site numbers correspond to the sequence alignment generated by " + \
                        d + ".  Sites with gaps are not shown.</p>\n"
                    out += "<p>Download: <a href='../../" + \
                        ancdir + "/" + f + "'>spreadsheet</a></p>"
                    out += "<div align='left'>"
...
def write_anccomp_indi(pair, con, ap):
    ...
    fout.write("<h2>Functional Loci on the Branch " +
               pair[0] + " &#10142; " + pair[1] + "</h2>\n")
    ...
    fout.write("The amino acid posterior probability distributions from " +
               pair[0] + " and " + pair[1])
    fout.write(
        " were compared at every sequence site, in order to detect statistical signatures of functional evolution. ")
    fout.write(
        "A composite metric, called &Delta;F, was used to compare probability distributions.")
    fout.write(
        " Sites with extreme &Delta;F scores (either positive or negative) should be considered strong hypotheses ")
    ...
    fout.write(
        " the protein's function. On the other hand, sites with negative &Delta;F scores indicate that ")
    ...
    fout.write(
        "<p>The following tables present the &Delta;F scores in multiple different formats.")
    fout.write("The &Delta;F score at each site is a composite of two metrics: (1) the KL Divergence (<em>i.e., k</em>) between the probability distributions of " +
               pair[0] + " and " + pair[1])
    ...
    fout.write("<p>&Delta;F Scores Ranked: <a href='../" +
               pair[0] + "to" + pair[1] + "/Df.details.txt'>text file</a></p>\n")
    # fout.write("<p>Scores by Site: <a href='../" + pair[0] + "to" + pair[1] + "/Df.details.txt'>spreadsheet</a></p>\n")

    fout.write("<p align='left'>&Delta;F Scores Across Sequence Sites: <a href='../" + pair[0] + "to" + pair[1] + "/Df-by-site.w=1.pdf'>pdf</a> | <a href='../" + pair[0] + "to" + pair[
               1] + "/Df-by-site.w=1.png'>png</a> | <a href='../" + pair[0] + "to" + pair[1] + "/Df-by-site.w=1.pdf.rscript'>R script</a> | <a href='../" + pair[0] + "to" + pair[1] + "/Df.ranked.txt'>spreadsheet</a></p>\n")
    fout.write("<a href='../" + pair[0] + "to" + pair[1] + "/Df-by-site.w=1.pdf'><img width='200px' src='../" + pair[
               0] + "to" + pair[1] + "/Df-by-site.w=1.png'></a>\n")
    fout.write("<br>\n")

    fout.write("<p align='left'>&Delta;F Scores Histogram: <a href='../" + pair[0] + "to" + pair[1] + "/Df-histogram.pdf'>pdf</a> | <a href='../" + pair[
               0] + "to" + pair[1] + "/Df-histogram.png'>png</a> | <a href='../" + pair[0] + "to" + pair[1] + "/Df-histogram.rscript'>R script</a></p>")
    fout.write("<a href='../" + pair[0] + "to" + pair[1] + "/Df-histogram.pdf'><img width='200px' src='../" + pair[
               0] + "to" + pair[1] + "/Df-histogram.png'></a>\n")
    fout.write("<br>\n")

    if ap.params["do_pdb_analysis"]:
        fout.write("<p align='left'>&Delta;F Scores on Structural Model of " + pair[1] + ": <a href='../" + pair[0] + "to" + pair[1] + "/pymol_script.scores.Df." + pair[
                   1] + ".pse'>pymol</a> | <a href='../" + pair[0] + "to" + pair[1] + "/pymol_ray.scores.Df." + pair[1] + ".png'>png</a>")
        fout.write("<br>")
        fout.write("<a href='../" + pair[0] + "to" + pair[1] + "/pymol_script.scores.Df." + pair[
                   1] + ".pse'><img width='100px' src='../" + pair[0] + "to" + pair[1] + "/pymol_ray.scores.Df." + pair[1] + ".png'></a>")
...
#     fout.write("</table>")
    ...
    for msa in get_alignment_method_names(con):
        for model in get_phylo_modelnames(con):
            if notos.path.exists(pair[0] + "to" + pair[1] + "/Df." + msa + "." + model + ".summary.html"):
                continue

            testpath = "../" + pair[0] + "to" + \
                pair[1] + "/Df-pdf-by-site.w=1.rscript"
            # if notos.path.exists( testpath ):
            #    continue
            ...
            fout.write("<div id='show" + msa + "." + model +
                       "' style='display:block; background:#ffff99;'>")
            fout.write(
                "<a tabindex='0' class='smalltext' onclick=\"toggle_visibility('tr" + msa + "." + model + "'); ")
            fout.write("    toggle_visibility('hide" +
                       msa + "." + model + "'); ")
            fout.write("    toggle_visibility('show" + msa +
                       "." + model + "');\">&darr; show</a></div>")
            fout.write("<div id='hide" + msa + "." + model +
            ...
            for metric in ["Df", "k", "p"]:
                fout.write("<td align='left' class='smalltext'>")
                fout.write("<p><a href='" + pair[0] + "to" + pair[
                           1] + "/Df." + msa + "." + model + ".summary.txt'>spreadsheet</a></p>")
                fout.write("</td>\n")
            fout.write("</tr>\n")
            ...
            fout.write(
                "<div width='100\%' align='left' style='display:none;' id='tr" + msa + "." + model + "'>\n")
            fout.write("<hr class='thinhr'>")

            fout.write("<div align='left'>\n")
            fin = open(pair[0] + "to" + pair[1] + "/Df." +
                       msa + "." + model + ".summary.html", "r")
...
def write_anccomp_plot(pair):
    ...
    for l in fin.xreadlines():
        if l.__len__() > 5 and notl.startswith("site"):
            tokens = l.split()
...
def write_css():
    ...
    fout.write("}\n")
    fout.write(
        "h4{ /* clickable links, usually underneath clickable icons*/\n")
    fout.write("font-size: 10pt;\n")
    fout.write("padding: 0;\n")
    fout.write("color: #444444;\n")
    fout.write("}\n")
    fout.write(
        "h5{ /* misc sub-titles, smaller than h2 and usually in-line */\n")
    fout.write("font-size: 12pt;\n")
    fout.write("color: #444444;\n")
    fout.write("}\n")
    fout.write("hr{\n")
    ...
    fout.write(
        ".headerrow{background: #C0C0C0; font-weight: bold; font-size: 12pt;}\n")
...
def write_mutations_indi(pair, con, ap):
    ...
    fout.write("<h2>Mutations on the Branch " +
               pair[0] + " &#10142; " + pair[1] + "</h2>\n")
    fout.write("<p>On the phylogenetic branch leading from " + pair[0] + " to " + pair[
               1] + ", several amino acid substitutions occurred. These substitutions can be classified into the following categories:</p>\n")

    fout.write("<ul>\n")
    fout.write("<li><strong>Type 1:</strong> the maximum likelihood (ML) amino acid changed between " + pair[0] + " and " + pair[
               1] + ", and there is poor support for " + pair[0] + "'s ML amino acid in " + pair[1] + ", and vice versa.</li>\n")
    fout.write("<li><strong>Type 2:</strong> the ML amino acid changed, but the ML residue in " +
               pair[0] + " is supported as an alternate residue in " + pair[1] + ", or vice versa.</li>\n")
    ...
    fout.write(
        "<li><strong>Indels:</strong> a residue was inserted or deleted in the sequence.</li>\n")
    fout.write("</ul>\n")
    ...
    fout.write(
        " which alignment and phylogenetic model were used to reconstruct the ancestors. If you wish to use only")
    fout.write(
        " one set of scores, then choose the alignment-model combination with the highest relative probability")
    fout.write(
        " (<em>Rel. Prob.</em>). The results can be downloaded as a tab-sepearted spreadsheet, or viewed as a")
    ...
    line_tokens = []
    for l in fin.xreadlines():
        if l.__len__() > 2 and notl.__contains__("Model"):
    ...
    for ii in range(0, line_tokens.__len__()):
        ...
        fout.write("<td align='center'><p>" +
                   tokens[2] + "</p></td>")  # indels
        fout.write("<td align='center'><p>" +
                   tokens[3] + "</p></td>")  # type 1
        fout.write("<td align='center'><p>" +
                   tokens[4] + "</p></td>")  # type 2
        # fout.write("<td align='center'><p>" + tokens[5] +  "</p></td>") #
        # type 3

        # Insert a column to download a spreadsheet with Type 1/2/3 scores for
        # every site in this msa-model combo.
        fout.write("<td align='center'>")
        fout.write("<p><a href=\"../" + pair[0] + "to" + pair[
                   1] + "/ancestral_changes." + msa + "." + model + ".txt\">spreadsheet</a>")
        if ap.params["do_pdb_analysis"]:
            fout.write(" | <a href=\"../" + pair[0] + "to" + pair[
                       1] + "/pymol_script.types." + msa + "." + model + "." + pair[1] + ".pse\">pymol</a>")
        fout.write("</p>")
        fout.write("</td>")
        ...
        # Div outer
        fout.write("<div width='100\%' align='left' style='display:none;' id='tr" +
                   msa + "." + model + "'>\n")
        ...
        fout.write("<div align='left'>\n")
        fin = open(pair[0] + "to" + pair[1] +
                   "/ancestral_changes." + msa + "." + model + ".html", "r")
        for seql in fin.xreadlines():
            fout.write(seql)
        fin.close()
        fout.write("</div>")
