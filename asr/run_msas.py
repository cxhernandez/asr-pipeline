...
from asr.asrpipelinedb import *
from asr.log import *
from asr.plots import *
from asr.phyloasr import *
...
def import_and_clean_erg_seqs(con, ap):
    ...
    if x is None:
        write_error(
            con, "I cannot find your original sequence path in the settings.")
        exit()
    ergseqpath = x[0]
...
def verify_erg_seqs(con, ap):
    ...
    cur.execute(sql)
    c = cur.fetchone()[0]
    write_log(con, "There are " + c.__str__() +
    ...
    c = cur.fetchone()[0]
    write_log(con, "There are " + c.__str__() +
              " amino acid sequences in the database.")
    ...
    if seeds is not None:
        for s in seeds:
            ...
            x = cur.fetchone()[0]
            if x == 0:
                write_error(con, "I cannot find your seed taxa '" +
                            s + "' in your original sequences.")
                exit()
    ...
    for ingroup in ap.params["ingroup"]:
        if ingroup not in ap.params["asrseedtaxa"]:
            write_error(
                con, "You did not specify an ASRSEED for the ingroup " + ingroup)
            exit()
        ...
        if seed_taxonid is None:
            write_error(con, "Your definition of ingroup " + ingroup + " uses the seed taxon " +
                        groupseed + ", but I cannot find that taxon in your sequences.")
            exit()
        ...
        for t in taxa:
            ...
            if taxonid is None:
                write_error(con, "Your definition of ingroup " + ingroup + " includes the taxon " +
                            t + ", but I cannot find this taxon in your sequences.")
                exit()
        ...
        sql = "select count(*) from GroupSeedTaxa where groupid=" + \
            groupid.__str__() + " and seed_taxonid=" + seed_taxonid.__str__()
        cur.execute(sql)
        count = cur.fetchone()[0]
        if count == 0:
        ...
        for t in taxa:
            taxonid = get_taxonid(con, t)
            sql = "select count(*) from GroupsTaxa where groupid=" + \
                groupid.__str__() + " and taxonid=" + taxonid.__str__()
            cur.execute(sql)
            count = cur.fetchone()[0]
            if count == 0:
    ...
    for t in taxa:
        taxonid = get_taxonid(con, t)
        if taxonid is None:
            write_error(con, "Your definition of outgroup includes the taxon " +
                        t + ", but I cannot find this taxon in your sequences.")
            exit()
    ...
    if ap.params["start_motif"] is not None:
        ...
        for s in seeds:
            ...
            if taxonid is None:
                write_error(
                    con, "I cannot find the Taxon ID for seed " + s + ". Error 330.")
                exit()
            s = get_original_seq(con, taxonid)
            if s.__contains__(m):
                foundit = True
        if not foundit:
            write_error(con, "I cannot find your desired starting motif ('" +
                        m + "') in any of your seed taxa.")
            exit()
        add_setting_value(con, "start_motif", m, unique=True)
    if ap.params["end_motif"] is not None:
        ...
        for s in seeds:
            ...
            if s.__contains__(m):
                foundit = True
        if not foundit:
            write_error(con, "I cannot find your desired ending motif ('" +
                        m + "') in any of your seed taxa.")
            exit()
...
def write_msa_commands(con):
    ...
    if es is None:
        write_error(
            con, "I cannot find your original sequence path in the settings. Error 229")
        exit()
    ergseqpath = es[0]
    ...
    for ii in x:
        ...
        if msa == "muscle":
            fout.write(exe + " -maxhours 5 -in " + ergseqpath +
                       " -out " + get_fastapath(msa) + "\n")
        elif msa == "prank":
            fout.write(exe + " -d=" + ergseqpath +
                       " -o=" + get_fastapath(msa) + "\n")
        elif msa == "msaprobs":
            fout.write(exe + " -num_threads 4 " + ergseqpath +
                       " > " + get_fastapath(msa) + "\n")
        elif msa == "mafft":
            fout.write(exe + " --thread 4 --auto " + ergseqpath +
                       " > " + get_fastapath(msa) + "\n")
...
def check_aligned_sequences(con):
    ...
    for ii in x:
        ...
        if not os.path.exists(expected_fasta):
            write_error(con, "I can't find the expected FASTA output from " +
                        msa + " at " + expected_fasta, code=None)
        taxa_seq = read_fasta(expected_fasta)

        """Does this alignment contain sequences?"""
        if taxa_seq.__len__() == 0:
            write_error(
                con, "I didn't find any sequences in the alignment from " + msa)
            exit()
        ...
        cur = con.cursor()
        sql = "select count(*) from AlignedSequences where almethod=" + \
            msaid.__str__() + " and datatype=1"
        cur.execute(sql)
        c = cur.fetchone()[0]
        write_log(con, "There are " + c.__str__() +
                  " sequences in the " + msa + " alignment.")
        ...
        if x is not None:
            for taxa in taxa_seq:
                ...
                aligned_codon_seq = align_codon_to_aaseq(
                    con, taxa_seq[taxa], org_codon_seq)
                # print "\n. 361:", taxa
                # print "nt:", aligned_codon_seq
                # print "aa:", taxa_seq[taxa]
                if aligned_codon_seq is not None:
                    import_aligned_seq(con, taxonid, msaid,
                                       aligned_codon_seq, datatype=0)
                else:
                    write_error(
                        con, "I cannot match the codon sequence and amino acid sequence for " + taxa)
                    exit()
    ...
    for aa in almethod_taxa_seq:
        for bb in almethod_taxa_seq:
            ...
            for t in taxaids:
                ...
                if matches is None:
                    write_log(con, "I found no matches between sequences " +
                              aa + " and " + bb + " for taxon " + t)
                for m in matches:
                    sql = "insert into SiteMap(taxonid, almethod1, site1, almethod2, site2)"
                    sql += " values(" + t.__str__() + "," + aa.__str__() + "," + \
                        m[0].__str__() + "," + bb.__str__() + \
                        "," + m[1].__str__() + ")"
                    cur.execute(sql)
                con.commit()
                sql = "select count(*) from SiteMap where almethod1=" + \
                    aa.__str__() + " and almethod2=" + bb.__str__()
                sql += " and taxonid=" + t.__str__()
                cur.execute(sql)
                countmapped = cur.fetchone()[0]
                write_log(con, "I mapped " + countmapped.__str__() + " sites between alignments " +
                          aa.__str__() + " and " + bb.__str__() + " for taxon " + t.__str__())
...
def fasta_to_phylip(inpath, outpath):
    ...
    for l in fin.xreadlines():
        if l.startswith(">"):
            taxa = re.sub(">", "", l)
            # trim any lingering GenBank labels. As of 03/2014, this line may
            # be unecessary - split()[0] part of the parsing occurs when the
            # RAW sequences are read into a FASTA file
            taxa = taxa.split()[0]
            taxa = taxa.strip()
            last_taxa = taxa
            taxa_seq[taxa] = ""
    ...
    fout = open(outpath, "w")
    fout.write("  " + taxa_seq.__len__().__str__() + "  " +
               taxa_seq[taxa_seq.keys()[0]].__len__().__str__() + "\n")
    for taxa in taxa_seq:
...
def write_alignment_for_raxml(con):
    ...
    for ii in x:
        ...
        sql += "(select treeid from ZorroThreshFasttree where almethod=" + \
            msaid.__str__() + ")"
        cur.execute(sql)
        y = cur.fetchall()
        best_treeid = y[0][0]
        ...
        if best_treeid is None:
            write_error(
                con, "RAxML cannot be run until a ZORRO threshold has been established.")
            exit()
        ...
        sql = "select min_score from ZorroThreshStats where thresh=" + \
            best_thresh.__str__() + " and almethod=" + msaid.__str__()
        cur.execute(sql)
        ...
        sql = "select site from AlignmentSiteScores where almethodid=" + \
            msaid.__str__() + " and scoringmethodid=" + zorromethod.__str__()
        ...
        sql = "select taxonid, alsequence from AlignedSequences where almethod=" + \
            msaid.__str__() + " and datatype=1"
        cur.execute(sql)
        y = cur.fetchall()
        pfout.write(y.__len__().__str__() + "   " +
                    sites.__len__().__str__() + "\n")
        for jj in y:  # for each sequence
            ...
            for c in finalseq:
                if c != "-":
                    hasdata = True
            if hasdata:  # Does this sequence contain at least one non-indel character?
                ffout.write(">" + taxa + "\n" + finalseq + "\n")
                pfout.write(taxa + "    " + finalseq + "\n")
            else:
                write_log(con, "After trimming the alignment " + msaid.__str__() + " to the ZORRO threshold, the taxa " +
                          taxa + " contains no sequence content. This taxa will be obmitted from downstream analysis.")
        ffout.close()
        pfout.close()
    ...
    for ii in x:
        ...
        if seeds is not None:
            for seed in seeds:
                ...
                [start, stop] = get_boundary_sites(
                    seedseq, start_motif, end_motif)
                if minstart is None:
                    minstart = start
                if maxstop is None:
                    maxstop == stop
        ...
        if maxstop is None:
            sql = "select max(length(alsequence)) from AlignedSequences where almethod=" + \
                alid.__str__()
            cur.execute(sql)
        ...
        sql += sitesetid.__str__() + "," + alid.__str__() + "," + \
            start.__str__() + "," + stop.__str__() + ")"
        cur.execute(sql)
        con.commit()
        ...
        sql = "select taxonid, alsequence from AlignedSequences where almethod=" + \
            alid.__str__() + " and datatype=1"
        cur.execute(sql)
        y = cur.fetchall()
        pfout.write(y.__len__().__str__() + "   " +
                    (stop - start + 1).__str__() + "\n")
        for jj in y:  # for each sequence
            taxonid = jj[0]
            taxa = get_taxon_name(con, taxonid)
            seq = jj[1]
            ...
            if hasdata:  # Does this sequence contain at least one non-indel character?
                ...
                write_log(con, "After trimming the alignment " + alid.__str__() + " to the seed boundaries, the taxa " +
                          taxa + " contains no sequence content. This taxa will be obmitted from downstream analysis.")
        ffout.close()
        pfout.close()
...
def build_zorro_commands(con):
    ...
    for ii in x:
        ...
        c = zorro_exe + " -sample " + \
            get_trimmed_fastapath(almethod) + " > " + \
            get_trimmed_fastapath(almethod) + ".zorro"
        z_commands.append(c)
...
def import_zorro_scores(con):
    ...
    for ii in x:
        ...
        for line in fin.xreadlines():
            ...
            sql += " VALUES(" + alid.__str__() + "," + zorroid.__str__() + \
                "," + site.__str__() + "," + score.__str__() + ")"
        ...
        sql = "select count(*) from AlignmentSiteScores where almethodid=" + \
            alid.__str__() + " and scoringmethodid=" + zorroid.__str__()
        cur.execute(sql)
        count = cur.fetchone()[0]
        write_log(con, "I found " + count.__str__() +
                  " ZORRO site scores for the alignment " + almethod.__str__())
...
def plot_zorro_stats(con):
    ...
    for ii in x:
        ...
        histogram(scores.values(), alname + "/zorro_scores",
                  xlab="ZORRO score", ylab="proportion of sites")
...
def build_fasttree4zorro_commands(con):
    ...
    for ii in x:
        alid = ii[0]
        alname = ii[1]
        commands += get_fasttree_zorro_commands_for_alignment(
            con, alid, zorroid)
...
def get_fasttree_zorro_commands_for_alignment(con, almethod, scoringmethodid):
    ...
    # debug moment
    # print "379:", almethod, min( site_scores.keys() ), max(
    # site_scores.keys() )
    ...
    for t in thresholds:
        ...
        if sitesetid is not None:
            sql = "delete from SiteSetsAlignment where setid=" + \
                sitesetid.__str__() + " and almethod=" + almethod.__str__()
            cur.execute(sql)
        elif sitesetid is None:
            """If there isn't a siteset for this threshold, then add one."""
            sql = "insert into SiteSets (setname) VALUES('" + \
                sitesetname + "')"
            cur.execute(sql)
            sitesetid = get_sitesetid(con, sitesetname)
        con.commit()
        ...
        for site in use_these_sites:
            ...
            sql += " VALUES(" + sitesetid.__str__() + "," + almethod.__str__() + \
                "," + site.__str__() + "," + site.__str__() + ")"
            cur.execute(sql)
        con.commit()
        write_log(con, "ZORRO details for " + alname + " -- threshold: " + t.__str__() +
                  ", min score:" + min_score.__str__() + ", Nsites: " + use_these_sites.__len__().__str__())

        sql = "insert into ZorroThreshStats(almethod, thresh, min_score, nsites)"
        sql += " VALUES(" + almethod.__str__() + "," + t.__str__() + "," + \
            min_score.__str__() + "," + use_these_sites.__len__().__str__() + ")"
        cur.execute(sql)
        con.commit()

        """Write a PHYLIP with the aligned sequences containing only the threshold-ed sites."""
        taxa_alseqs = get_sequences(con, almethod=almethod, sitesets=[
                                    sitesetid])  # sites = use_these_sites)
        seqs = {}
...
def measure_fasttree_distances(con):
    ...
    for i in range(0, ids.__len__()):
        ...
        for j in range(i, ids.__len__()):
            ...
            ii_jj_symd[ii][jj] = treeid_dendro[ii].symmetric_difference(
                treeid_dendro[jj]) + treeid_dendro[jj].symmetric_difference(treeid_dendro[ii])
            sql = "insert into FasttreeSymmetricDistances(treeid1, treeid2, distance) VALUES("
            sql += ii.__str__() + "," + jj.__str__() + "," + \
                ii_jj_symd[ii][jj].__str__() + ")"
            cur.execute(sql)
            ii_jj_rfd[ii][jj] = treeid_dendro[
                ii].robinson_foulds_distance(treeid_dendro[jj])
            sql = "insert into FasttreeRFDistances(treeid1, treeid2, distance) VALUES("
            sql += ii.__str__() + "," + jj.__str__() + "," + \
                ii_jj_rfd[ii][jj].__str__() + ")"
            cur.execute(sql)
...
def analyze_fasttrees_for_msa(con, almethod):
    ...
    for t in thresholds:
        ...
        if not os.path.exists(thresh_treepath[t]):
            write_error(con, "I can't find the FastTree tree for the ZORRO trial " +
                        t.__str__() + " at path " + thresh_treepath[t])
            exit()
        ...
        fin.close()
        newick = newick.strip()
        sql = "insert into ZorroThreshFasttree(almethod, thresh, newick) VALUES(" + \
            almethod.__str__()
        ...
        sql = "select treeid from ZorroThreshFasttree where almethod=" + \
            almethod.__str__() + " and thresh=" + t.__str__()
        cur.execute(sql)
        ...
        sql += "VALUES(" + treeid.__str__() + "," + \
            mean_bs.__str__() + "," + sum_branches.__str__() + ")"
        cur.execute(sql)
        con.commit()

        write_log(con, "Zorro threshold stat: " + alname + " thresh:" + t.__str__() +
                  " branch_sum:" + sum_branches.__str__() + " mean_bs:" + mean_bs.__str__())
...
def compare_fasttrees_for_alignment(con, msaid):
    ...
    for ii in range(0, threshes.__len__()):
        ...
        for jj in range(0, threshes.__len__()):
            sql = "select distance from FasttreeRFDistances where treeid1=" + \
                thresh_treeid[threshes[ii]].__str__(
                ) + " and treeid2=" + thresh_treeid[threshes[jj]].__str__()
            cur.execute(sql)
            d = cur.fetchone()
            if d is None:
                sql = "select distance from FasttreeRFDistances where treeid1=" + \
                    thresh_treeid[threshes[jj]].__str__(
                    ) + " and treeid2=" + thresh_treeid[threshes[ii]].__str__()
                cur.execute(sql)
                d = cur.fetchone()
            line += "%.1f\t" % d
        # print line
    ...
    for ii in range(0, threshes.__len__()):
        line = threshes[ii].__str__() + "\t"
        for jj in range(0, threshes.__len__()):
            sql = "select distance from FasttreeSymmetricDistances where treeid1=" + \
                thresh_treeid[threshes[ii]].__str__(
                ) + " and treeid2=" + thresh_treeid[threshes[jj]].__str__()
            cur.execute(sql)
            d = cur.fetchone()
