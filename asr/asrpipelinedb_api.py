...
def add_checkpoint(con, checkpoint):
    cur = con.cursor()
    seconds = time.time()
    sql = "insert or replace into CheckPoints (checkpoint, time) values(" + \
        checkpoint.__str__() + "," + seconds.__str__() + ")"
    cur.execute(sql)
    con.commit()
...
def import_original_seq(con, shortname, sequence, datatype=1):
    ...
    if count == 0:
        sql = "insert into Taxa (fullname, shortname) VALUES('" + \
            shortname + "','" + shortname + "')"
        cur.execute(sql)
        con.commit()
    ...
    sql = "select count(*) from OriginalSequences where taxonid=" + \
        taxonid.__str__() + " and datatype=" + datatype.__str__()
    cur.execute(sql)
    count = cur.fetchone()[0]
    if count == 0:
        sql = "insert into OriginalSequences (taxonid, sequence, datatype) VALUES(" + \
            taxonid.__str__() + ",'" + sequence + "'," + datatype.__str__() + ")"
        cur.execute(sql)
        con.commit()
...
def import_aligned_seq(con, taxonid, almethodid, seq, datatype=1):
    cur = con.cursor()
    sql = "select count(*) from AlignedSequences where taxonid=" + taxonid.__str__() + \
        " and almethod=" + almethodid.__str__() + " and datatype=" + datatype.__str__()
    cur.execute(sql)
    count = cur.fetchone()[0]
    if count == 0:
...
def import_alignment_method(con, name, exe):
    ...
    if count == 0:
        ...
        sql = "update  AlignmentMethods set exe_path='" + \
            exe + "' where id=" + id.__str__()
        cur.execute(sql)
        con.commit()
...
def get_ancestorid(con, alias, almethod, phylomodel):
    cur = con.cursor()
    sql = "select ancid from AncestorsAlias where alias='" + alias + \
        "' and ancid in (select id from Ancestors where almethod=" + \
        almethod.__str__() + " and phylomodel=" + phylomodel.__str__() + ")"
    cur.execute(sql)
    x = cur.fetchall()
    if x.__len__() == 0:
        return None
...
def get_original_seq(con, taxonid, datatype=1):
    cur = con.cursor()
    sql = "select sequence from OriginalSequences where taxonid=" + \
        taxonid.__str__() + " and datatype=" + datatype.__str__()
    cur.execute(sql)
    x = cur.fetchall()
    if x.__len__() == 0:
...
def get_aligned_seq(con, taxonid, almethodid, datatype=1):
    cur = con.cursor()
    sql = "select alsequence from AlignedSequences where taxonid=" + taxonid.__str__() + \
        " and almethod=" + almethodid.__str__() + " and datatype=" + datatype.__str__()
    cur.execute(sql)
    x = cur.fetchall()
    if x.__len__() == 0:
...
def get_sequences(con, almethod=None, sitesets=[], sites=[], taxagroups=[], datatype=1):
    ...
    if almethod is None:
        ...
        if not is_valid_almethod(con, almethod):
    ...
    # Key = taxonid, value = aligned or original sequence, untrimmed.
    taxa_seqs = {}
    for ii in x:
        taxonid = ii[0]
        seq = ii[1]
    ...
    elif sites != [] and almethod is not None:
...
def is_valid_almethod(con, methodid):
    cur = con.cursor()
    sql = "select count(*) from AlignmentMethods where id=" + \
        methodid.__str__()
    cur.execute(sql)
    x = cur.fetchone()[0]
    if x > 0:
...
def get_taxaid_from_ancestor(con, ancid):
    cur = con.cursor()
    sql = "select taxonid from GroupsTaxa where groupid in (select ingroupid from AncestorsGroups where ancid=" + \
        ancid.__str__() + " )"
    cur.execute(sql)
    x = cur.fetchall()
    taxonids = []
...
def get_siteranges_in_set(con, sitesetid, almethodid):
    ...
    cur = con.cursor()
    sql = "select fromsite, tosite from SiteSetsAlignment where setid=" + \
        sitesetid.__str__() + " and almethod=" + almethodid.__str__()
    cur.execute(sql)
    x = cur.fetchall()
    siteranges = []
    for ii in x:
...
def write_phylip(con, seqs, ppath, firstseq=None):
    ...
    if firstseq is not None and firstseq in seqs:
        fout.write(firstseq + "   " + seqs[firstseq] + "\n")
    elif firstseq is not None:
        write_error(con, "Error 327: the seed sequence " +
                    firstseq + " cannot be found.")
        exit()
...
def get_alignmentsitescores(con, almethod, scoringmethoid):
    cur = con.cursor()
    sql = "select site, score from AlignmentSiteScores where almethodid=" + \
        almethod.__str__() + " and scoringmethodid=" + scoringmethoid.__str__()
    cur.execute(sql)
    y = cur.fetchall()
    site_scores = {}
    for jj in y:
        site_scores[jj[0]] = jj[1]
    # print "235: almethod", almethod, ", N sitescores=",
    # site_scores.__len__().__str__() + ", from " + min(
    # site_scores.keys()).__str__() + " to " + max(
    # site_scores.keys()).__str__()
    return site_scores
...
def add_dnds_test(con, almethodid, phylomodelid, ancid1, ancid2, dnds_modelid):
    ...
    cur = con.cursor()
    sql = "select count(*) from DNDS_Tests where almethod=" + \
        almethodid.__str__()
    sql += " and phylomodel=" + phylomodelid.__str__() + " and anc1=" + \
        ancid1.__str__()
    sql += " and anc2=" + ancid2.__str__() + " and dnds_model=" + \
        dnds_modelid.__str__()
    cur.execute(sql)
    count = cur.fetchone()[0]
    if count == 0:
        sql = "insert into DNDS_Tests (almethod, phylomodel, anc1, anc2, dnds_model)"
        sql += " values(" + almethodid.__str__() + "," + \
            phylomodelid.__str__() + "," + ancid1.__str__()
