...
def run_similarity_network_analysis(con):
    ...
    if not os.path.exists("pythoscape"):
    ...
    if blastp_exe is None:
        write_log(
            con, "I can't find your BLAST executable. I'm skipping the similarity network analysis.")
        return
    blastp_exe = blastp_exe[0]
    cdhit_exe = get_setting_values(con, "cdhit_exe")
    if cdhit_exe is None:
        write_log(
            con, "I canot find your CD-HIT executable. I'm skipping the similarity network analysis.")
        return
    ...
    plugin_2 = a_l_b.AddBLASTEdgesFromLocalBLAST(
        "blastp", include_atts=['bit_score', '% id', 'alignment_identities', '-log10(E)'])
    plugin_4 = m_c_r.CreateCDHITRepnodes(
        network_name, "cd-hit", c=0.7, n=5, repnode_att=repnode_att)
    ...
    plugin_6 = a_r_e.AddEdgesToRepnodeNetwork(
        network_name, repnode_att, '% id', filt_dir='>', filt_value=0.4)
    plugin_7 = a_r_a.AddAttributesByIfAny(
        network_name, repnode_att, 'family', 'repnode family')
    xgmmlpath = "pythoscape/" + network_name + "-" + repnode_att + '20.xgmml'
    out_plugin_1 = o_x.OutputXGMML(xgmmlpath, 'sigma gst representative node network @ 20',
                                   filt_name='rep-net mean', filt_dir='>', filt_value=20)
    xgmmlpath = "pythoscape/" + network_name + "-" + repnode_att + '50.xgmml'
    out_plugin_2 = o_x.OutputXGMML(xgmmlpath, 'sigma gst representative node network @ 50',
                                   filt_name='rep-net mean', filt_dir='>', filt_value=50)
    xgmmlpath = "pythoscape/" + network_name + "-" + repnode_att + '100.xgmml'
    out_plugin_3 = o_x.OutputXGMML(xgmmlpath, 'sigma gst representative node network @ 100',
                                   filt_name='rep-net mean', filt_dir='>', filt_value=100)
    outpath = "pythoscape/" + network_name + "-" + \
        repnode_att + '.OutputIdentifierTable.txt'
