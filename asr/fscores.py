...
def get_dnds_commands(con):
    ...
    for msa in get_alignment_method_names(con):
        ...
        for model in get_phylo_modelnames(con):
            ...
            sql = "select newick from UnsupportedMlPhylogenies where almethod=" + \
            ...
            for pair in get_ancestral_comparison_pairs(con):
                ...
                if ancid1 == ancid2:
                    write_log(con, "I'm skipping dnds test for model=" + model +
                              ", msa=" + msa.__str__() + " because anc1 and anc2 match.")
                    continue
                ...
                for dnds_model in dnds_models:
                    ...
                    fout.write(codeml_exe + " " + get_setting_values(con,
                                                                     "geneid")[0] + ".ctl > catch.out\n")
                    fout.write("cd -\n")
                    fout.close()
...
def get_dnds_directory(con, testid):
    ...
    dndsdir = "dnds/dnds." + alname.__str__() + "." + phylomodelname.__str__() + "." + \
        ancid1name.__str__() + "." + ancid2name.__str__() + \
        "." + dnds_modelname.__str__()
    return dndsdir
...
def write_codeml_tree(con, testid):
    ...
    if dnds_model == "M0":
        sql = "select newick from UnsupportedMlPhylogenies where almethod=" + \
            almethod.__str__() + " and phylomodelid=" + phylomodel.__str__()
    else:
...
def write_codeml_alignment(con, testid):
    ...
    outdir = get_dnds_directory(con, testid)
    phylippath = outdir + "/" + \
        get_setting_values(con, "geneid")[0] + ".codons.phylip"
    ...
    sql = "select taxonid, alsequence from AlignedSequences where almethod=" + \
        almethod.__str__() + " and datatype=0"
    cur.execute(sql)
    x = cur.fetchall()
    seqs = {}
...
def write_dnds_control_file(con, testid):
    cur = con.cursor()
    sql = "select name from DNDS_Models where id in (select dnds_model from DNDS_Tests where id=" + \
        testid.__str__() + ")"
    cur.execute(sql)
    x = cur.fetchall()
    if x is None:
...
def get_neb_scores_from_rstfile(con, rstpath, testid, neb_sig_sites):
    ...
    for l in lines:
        ...
        if found_neb and not l.startswith("(amino") and not done:
            if l.__len__() > 2:
                tokens = l.split()
        ...
        if found1 and not done:
            ...
            if tokens.__len__() == expected_columns:
                ...
                if not tokens[4].startswith("(") and not tokens[4].startswith("+"):
                    p3 = float(re.sub("\*", "", tokens[4]))
                if tokens.__len__() > 5:
                    if not tokens[5].startswith("(") and not tokens[5].startswith("+"):
                ...
                sql = "select pp, state from AncestralStates" + \
                    ancid1.__str__() + " where site=" + site.__str__()
                cur.execute(sql)
                x = cur.fetchall()
                maxpp1 = 0.0
                state1 = None
                ...
                sql = "select pp, state from AncestralStates" + \
                    ancid2.__str__() + " where site=" + site.__str__()
                cur.execute(sql)
                x = cur.fetchall()
                maxpp2 = 0.0
                state2 = None
                ...
                sql += "," + p1.__str__() + "," + p2.__str__() + "," + p3.__str__() + "," + \
                    p4.__str__() + "," + ancmu.__str__() + "," + sig_flag.__str__() + ")"
                cur.execute(sql)
                con.commit()
...
def get_beb_scores_from_rstfile(con, rstpath, testid, beb_sig_sites):
    ...
    for l in lines:
        ...
        if found_beb and not l.startswith("(amino"):
            if l.__len__() > 2:
                tokens = l.split()
                site = re.sub(" ", "", tokens[0])
                if site.startswith("1"):
...
def parse_dnds_results(con):
    ...
    for ii in x:
        ...
        if not os.path.exists(catchpath):
            write_error(
                con, "I cannot find dN/dS output in the folder " + outdir)
            continue
        ...
        if lnl is None:
            write_error(
                con, "I couldn't find lnL value in the PAML output " + catchpath)
            continue

        # write_log(con, outdir + " Log-Likelihood = " + lnl.__str__())

        sql = "insert into DNDS_lnL(testid, lnl) values(" + \
            testid.__str__() + "," + lnl.__str__() + ")"
        cur.execute(sql)
        con.commit()
        ...
        sql += " values(" + testid.__str__() + "," + pclass1.__str__() + "," + \
            pclass2.__str__() + "," + pclass3.__str__() + "," + pclass4.__str__()
        sql += "," + wclass1.__str__() + "," + wclass2.__str__() + "," + \
            wclass3.__str__() + "," + wclass4.__str__() + ")"
        cur.execute(sql)
        con.commit()
        fin.close()
        ...
        if dnds_model.__contains__("sites"):
            ...
            for l in fin.xreadlines():
                ...
                if found_sig_sites and not l.__contains__("Pr") and l.__len__() > 1 and not l.__contains__("Naive Empirical Bayes (NEB)"):
            ...
            for l in fin.xreadlines():
                ...
                if found_sig_sites and not l.__contains__("Pr") and l.__len__() > 1 and not l.__contains__("Bayes Empirical Bayes (BEB)"):
                    if l.startswith("Positive"):
                        continue
                    if l.startswith("(amino acids"):
                        continue
            ...
            sql = "select count(*) from BEB_scores where testid=" + \
                testid.__str__()
            cur.execute(sql)
            count = cur.fetchone()[0]
            write_log(con, "I found " + count.__str__() +
