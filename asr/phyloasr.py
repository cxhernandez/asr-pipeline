...
def write_raxml_commands(con):
    ...
    for msa in get_alignment_method_names(con):
        ...
        for model in get_phylo_modelnames(con):
            ...
            if cv is None:
                write_error(
                    con, "I cannot find the executable path for raxml.")
                exit()
...
def check_raxml_output(con):
    ...
    for msa in get_alignment_method_names(con):
        ...
        for model in get_phylo_modelnames(con):
            ...
            if not os.path.exists(raxml_treepath):
                print "I can't find the expected result from RAxML at " + here + "/" + msa + "/RAxML_bestTree." + runid
                write_error(con, "I can't find the expected result from RAxML at " +
                            here + "/" + msa + "/RAxML_bestTree." + runid)
            ...
            sql += msaid.__str__() + "," + modelid.__str__() + \
                ",'" + rooted_treestring + "')"
            cur.execute(sql)
            con.commit()
...
def get_mlalpha_pp(con):
    ...
    for msa in get_alignment_method_names(con):
        ...
        for treeid in treeid_lnl:
            ...
            sql = "insert into TreePP (mltreeid, pp) values(" + \
                treeid.__str__() + "," + treeid_pp[treeid].__str__() + ")"
            cur.execute(sql)
            con.commit()
...
def calc_alrt(con):
    ...
    for msa in get_alignment_method_names(con):
        ...
        for model in get_phylo_modelnames(con):
            ...
            if x is None:
                write_error(
                    con, "I cannot find the path to the PhyML in your settings. Error 298")
                exit()
...
def calc_alr(con):
    ...
    for msa in get_alignment_method_names(con):
        for model in get_phylo_modelnames(con):
            ...
            if not os.path.exists(alrt_treepath):
            ...
            if not os.path.exists(alr_treepath):
...
def import_supported_trees(con):
    ...
    for ii in x:
        ...
        if not os.path.exists(alrt_treepath):
            write_error(
                con, "Oops, I cannot find the aLRT-supported tree at " + alrt_treepath)
            exit()
        if not os.path.exists(alr_treepath):
            write_error(
                con, "Oops, I cannot find the aLR-supported tree at " + alr_treepath)
            exit()
...
def get_asr_commands(con):
    ...
    for msa in get_alignment_method_names(con):
        for model in get_phylo_modelnames(con):
            ...
            if not os.path.exists(msa + "/asr." + model):
            ...
            asr_commands.append(lazaarus_exe + " --alignment " + fastapath + " --tree " + asrtreepath + " --model " + modelstr + " --outputdir " +
                                msa + "/asr." + model + " --branch_lengths fixed --asrv 8 --codeml --gapcorrect True --outgroup " + outgroup_string)
...
def check_asr_output(con):
    ...
    for msa in get_alignment_method_names(con):
        ...
        for model in get_phylo_modelnames(con):
            ...
            if not os.path.exists(cladopath):
                write_error(
                    con, "I cannot find the expected cladogram at " + cladopath)
                exit()
            ...
            sql = "insert into AncestralCladogram (unsupportedmltreeid,newick) values(" + \
                treeid.__str__() + ",\"" + cladostring + "\")"
            # print "528", sql
            cur.execute(sql)
            con.commit()
            ...
            if not os.path.exists(datpath):
                write_error(
                    con, "I cannot find reconstructed ancestors in the folder " + datpath)
                exit()

            """Parse each ancestral DAT file."""
            for d in os.listdir(datpath):
                if not d.endswith(".dat"):
                    continue
                ...
                sql += msaid.__str__() + "," + modelid.__str__() + \
                    ",'Node" + nodenum.__str__() + "')"
                cur.execute(sql)
                con.commit()

                """Get the ID of the ancestor we just inserted."""
                sql = "select id from Ancestors where almethod=" + msaid.__str__() + " and phylomodel=" + \
                    modelid.__str__() + " and name='Node" + nodenum.__str__() + "'"
                cur.execute(sql)
                ...
                sql = "create table if not exists " + \
                    tablename.__str__() + "(site INT, state CHAR, pp FLOAT)"
                cur.execute(sql)
                con.commit()
                ...
                for site in ssp:
                    for state in ssp[site]:
                        ...
                        sql = "insert or replace into " + \
                            tablename + " (site, state, pp) values("
                        sql += site.__str__() + ",'"
                        sql += state + "',"
                        sql += pp.__str__() + ")"
                        cur.execute(sql)
    ...
    for msa in get_alignment_method_names(con):
        for model in get_phylo_modelnames(con):
            ...
            for ing in ingroup_ids:
                ...
                getanc_commands.append(lazarus_exe + " --alignment " + asrmsa + " --tree " + asrtree + " --model " + modelstr + " --outputdir " +
                                       here + "/" + msa + "/asr." + model + " --outgroup " + outgroup_string + " --ingroup " + ingroup_string + " --getanc True")
                getanc_commands.append("mv " + msa + "/asr." + model + "/ancestor-ml.dat " +
                                       msa + "/asr." + model + "/" + ingroup_name + ".dat")
                getanc_commands.append("mv " + msa + "/asr." + model + "/ancestor.out.txt " +
                                       msa + "/asr." + model + "/" + ingroup_name + ".txt")
        ...
        for model in get_phylo_modelnames(con):
            ...
            if os.path.exists(logpath):
                ...
                if line1.__contains__("error") or line1.__contains__("Error"):
                    write_error(
                        con, "There was an error extracting ancestors:")
            ...
            for ingroupid in groupid_name:
                ancpath = msa + "/asr." + model + "/" + \
                    groupid_name[ingroupid] + ".txt"
                if not os.path.exists(ancpath):
                    write_error(
                        con, "I cannot find the expected ancestor " + ancpath)
                    exit()
                ...
                sql = "select id from Ancestors where name='Node" + this_node.__str__() + \
                    "' and almethod=" + msaid.__str__() + " and phylomodel=" + modelid.__str__()
                cur.execute(sql)
                x = cur.fetchall()
                if x.__len__() == 0:
                    write_error(
                        con, "Error 596 - I cannot find the entry in Ancestors for the ingroup ID " + ingroupid.__str__())
                    exit()
                ancid = x[0][0]

                sql = "insert into AncestorsAlias (ancid, alias) values(" + ancid.__str__(
                ) + ",'" + groupid_name[ingroupid] + "')"
                sql
                cur.execute(sql)
                con.commit()

                sql = "insert into AncestorsGroups (ancid, ingroupid, outgroupid) values(" + ancid.__str__(
                ) + "," + ingroupid.__str__() + "," + outgroupid.__str__() + ")"
                cur.execute(sql)
                con.commit()
...
def setup_pdb_maps(ap):
    ...
    if not os.path.exists(os.getcwd() + "/" + ap.params["phyre_out"]):
        print "\n. I can't find your PHYRE output folder at", os.getcwd() + "/" + ap.params["phyre_out"]
        print "\n. Check your configuration file and try again.\n"
        exit()
    if not os.path.exists(os.getcwd() + "/" + ap.params["phyre_out"] + "/summaryInfo"):
    ...
    fin = open(os.getcwd() + "/" +
               ap.params["phyre_out"] + "/summaryInfo", "r")
    ...
    for l in lines:
        if l.__len__() > 2:
            ...
            pdbpath = os.getcwd() + "/" + \
                ap.params["phyre_out"] + "/" + tokens[2] + ".final.pdb"
        ...
        for msa in get_alignment_method_names(con):
            ...
            for model in get_phylo_modelnames(con):
                ...
                if ancid1 == ancid2:
                    ...
                    write_log(con, "I'm skipping the Fscore analysis for " + msa + "." + model +
                              " " + pair.__str__() + " because the ancestors are the same node.")
                    continue
                ...
                sql = "select pp from TreePP where mltreeid in (select id from UnsupportedMlPhylogenies where almethod=" + msaid.__str__(
                ...
                comparelines += "compare " + msa + "/asr." + model + "/" + \
                    pair[0] + ".dat " + msa + "/asr." + model + \
                    "/" + pair[1] + ".dat " + runid + "\n"
                weightlines += "msaweight " + runid + " " + pp.__str__() + "\n"

                sql = "insert into FScore_Tests (almethod, phylomodel, ancid1, ancid2)"
                sql += " values(" + msaid.__str__() + "," + modelid.__str__() + \
                    "," + ancid1.__str__() + "," + ancid2.__str__() + ")"
                cur.execute(sql)
                con.commit()
    ...
    for pair in pairs:
        ...
        for msa in get_alignment_method_names(con):
            ...
            for modelname in get_phylo_modelnames(con):
                ...
                sql = "select id from FScore_Tests where almethod=" + \
                    msaid.__str__() + " and phylomodel=" + modelid.__str__()
                ...
                if not os.path.exists(summary_path):
                    write_error(
                        con, "I cannot find your Df output file at " + summary_path)
                    continue
                ...
                if not os.path.exists(summary_path):
                    write_error(
                        con, "I cannot find your p output file at " + summary_path)
                    continue
                fin = open(summary_path, "r")
                lines = fin.readlines()
                for l in lines:
                ...
                if not os.path.exists(summary_path):
                    write_error(
                        con, "I cannot find your k output file at " + summary_path)
                    continue
                fin = open(summary_path, "r")
                lines = fin.readlines()
                for l in lines:
                ...
                for site in site_df:
                    ...
                    sql += " values(" + testid.__str__() + "," + site.__str__() + \
                        "," + df.__str__() + "," + k.__str__() + "," + p.__str__() + ")"
                    cur.execute(sql)

                con.commit()

                sql = "select count(*) from FScore_Sites where testid=" + \
                    testid.__str__()
                cur.execute(sql)
                count = cur.fetchone()[0]

                write_log(con, "I found " + count.__str__() +
                          " F-scored sites for test ID " + testid.__str__())
...
def get_branch_supports(treepath):
    ...
    for line in fin.readlines():
        tokens = line.split(":")
        for t in tokens:
            if t.__contains__(")") and not t.__contains__(";"):
                ts = t.split(")")
                if ts[1] != '':
                    support = float(ts[1])
...
def compute_tree_distances(con):
    cur = con.cursor()
    cur.execute(
        "insert or replace into TreeDistanceMetrics(metricid, name) values(1, 'symmetric')")
    cur.execute(
        "insert or replace into TreeDistanceMetrics(metricid, name) values(2, 'euclidean')")
    con.commit()
