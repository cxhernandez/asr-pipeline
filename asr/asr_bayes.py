        ...
        for jj in y:
            ...
            for a in ap.params["ingroup"]:
                ...
                if not os.path.exists(ancdir):
                    continue
                if not os.path.exists(ancdir + "/BAYES_SAMPLES"):
                ...
                for f in os.listdir(ancdir):
                    if f.__contains__(".dat"):
                        ...
                        data = get_pp_distro(
                            alname + "/asr." + modelname + "/tree1/" + f)
                        if data.keys().__len__() < 1:
                            print "I found no data in the ancestral file ", f
                        n = 100
                        fout = open(alname + "/asr." + modelname +
                                    "/tree1/BAYES_SAMPLES/bayes." + f, "w")
                        mls = get_ml_sequence_from_file(
                            alname + "/asr." + modelname + "/tree1/" + f)
                        shortname = f.split(".")[0]
                        ...
                        for ii in range(0, n):
                            fout.write(
                                ">" + shortname + ".posterior.sample." + alname.__str__() + "\n")
                            l = sample_data(data)
                            fout.write(l + "\n")
