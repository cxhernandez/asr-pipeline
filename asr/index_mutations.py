        ...
        for modelid in get_phylo_modelids(con):
            ...
            for edge in t.preorder_edge_iter():
                if edge.head_node is None or edge.tail_node is None:
                    continue
                if edge.head_node.label is None or edge.tail_node.label is None:
                    continue
...
def index_mutations_helper(con, msaid, phylomodelid, anc1name, anc2name):
    ...
    ancid1 = None
    ancid2 = None
    sql = "select name, id from Ancestors where name='" + anc1name + \
        "' and almethod=" + msaid.__str__() + " and phylomodel=" + \
        phylomodelid.__str__()
    cur.execute(sql)
    x = cur.fetchone()
    if x is not None:
        ancid1 = x[1]
    sql = "select name, id from Ancestors where name='" + anc2name + \
        "' and almethod=" + msaid.__str__() + " and phylomodel=" + \
        phylomodelid.__str__()
    cur.execute(sql)
    x = cur.fetchone()
    ...
    mutation_header = []
    mutation_rows = []
    # each item is a tuple: (site in msa, site in seed, ancestral state 1,
    # anc. state 2, weight)
    mutation_shortlist = []
    ...
    for site in range(1, nsites + 1):
        ...
        for match in matched_ancestors:
            ...
            # Six values for the Django template to use. see the line down
            # below starting with this_column = (
            this_column = ("", "", "", "", "", "")
