...
def read_config_file(con, ap):
    ...
    for l in fin.xreadlines():
        ...
        # user-specified sequence alignments
        elif tokens[0].startswith("USER_ALIGNMENT"):
            if "user_msas" not in ap.params:
                ap.params["user_msas"] = {}
            x = tokens[1].split()
            if x.__len__() < 2:
                write_error(
                    con, "This line in your configuration file appears to be too short: " + l, code=None)
            msaname = x[0]
            if ap.params["user_msas"].__contains__(msaname):
                write_error(
                    con, "Your configuration appears to contain duplicated lines for the user-specified alignment named " + msaname)
            if ap.params["msa_algorithms"].__contains__(msaname):
                write_error(con, "Your configuration file contains a user-specified alignment, named " + msaname +
                            ", that conflicts with the name of a built-in alignment algorithm. Please re-name your user-specified alignment and try again.")
                exit()
            msapath = x[1]
            if not os.path.exists(msapath):
                write_error(con, "I cannot find the user-specified alignment named " +
                            msaname + " at path: " + msapath)
                exit()
            else:
            ...
            if ap.params["start_motif"].__contains__("None"):
                ...
                write_log(con, "Warning: your start motif ('" +
                          ap.params["start_motif"] + "') is very short.")
            ...
            if ap.params["end_motif"].__contains__("None"):
                ...
                write_log(con, "Warning: your end motif ('" +
                          ap.params["end_motif"] + "') is very short.")
            ...
            if not os.path.exists(constraint_tree):
                ...
                exit()
            add_setting_value(con, "constraint_tree",
                              constraint_tree, unique=True)

        elif tokens[0].startswith("FAMILY_DESCRIPTION"):
            ap.params["family_description"] = tokens[1]
            add_setting_value(con, "family_description",
                              ap.params["family_description"])
            ...
            # tolerate and remove whitespace.
            ingroup = re.sub(" ", "", ingroup)
        ...
        # map Df scores for this ancestor onto the PDB path
        elif tokens[0].startswith("MAP2PDB"):
            anc = tokens[0].split()[1]
...
def verify_config(con, ap):
    ...
    if x is None:
        write_error(
            con, "You did not specify SEQUENCES, a path to the original sequences, in your configuration file.")
        exit()
    ...
    if x is not None:
        if not os.path.exists(x[0]):
            write_error(
                con, "I cannot find your nucleotide FASTA file at " + x[0])
    ...
    if "ingroup" not in ap.params:
        ...
        for a in ap.params["ingroup"]:
            ...
            if a not in ap.params["asrseedtaxa"]:
                ...
                write_error(
                    con, "You did not specify a SEED for the ancestor " + a)
                exit()
        ...
        for c in ap.params["compareanc"]:
            ...
            if a1 not in ap.params["ingroup"]:
                ...
                write_error(con, "you specified a comparison between ancestors " + a1,
                            " and " + a2 + " but " + a1 + " was not defined in the ANCESTORS line.")
                exit()
            if a1 not in ap.params["ingroup"]:
                print "\n. ERROR: you specified a comparison between ancestors", a1, "and", a2, "but", a2, "was not defined in the ANCESTORS line."
                write_error(con, "you specified a comparison between ancestors " + a1,
                            " and " + a2 + " but " + a2 + " was not defined in the ANCESTORS line.")
                exit()

            sql = "insert or replace into CompareAncestors (alias1, alias2) values('" + \
                a1 + "','" + a2 + "')"
            cur.execute(sql)
            con.commit()

    ergseqpath = get_setting_values(con, "ergseqpath")[0]
    if not os.path.exists(ergseqpath):
    ...
    for msa in ap.params["msa_algorithms"]:
        if msa == "muscle" and "muscle_exe" not in ap.params:
            ...
            write_error(
                con, "Something is wrong. Your config file doesn't have an executable path for MUSCLE.")
            exit()
        ...
        if msa == "msaprobs" and "msaprobs_exe" not in ap.params:
            ...
            write_error(
                con, "Something is wrong. Your config file doesn't have an executable path for MSAPROBS.")
            exit()
        ...
        if msa == "prank" and "prank_exe" not in ap.params:
            ...
            write_error(
                con, "Something is wrong. Your config file doesn't have an executable path for PRANK.")
            exit()
        ...
        if msa == "mafft" and "mafft_exe" not in ap.params:
            ...
            write_error(
                con, "Something is wrong. Your config file doesn't have an executable path for MAFFT.")
            exit()
    ...
    for msaname in ap.params["user_msas"]:
        ...
        if not os.path.exists(msaname):
            os.system("mkdir " + msaname)
        os.system("cp " + ap.params["user_msas"]
                  [msaname] + " " + get_fastapath(msaname))
    ...
    if "zorro_thresholds" not in ap.params:
        ap.params["zorro_thresholds"] = [0.05, 0.1, 0.15,
                                         0.2, 0.25, 0.3, 0.35, 0.5, 0.6, 0.75, 1.0]
...
def verify_all_exe(con):
    ...
    """ ZORRO """
    if not ap.getOptionalToggle("--skip_zorro"):
    ...
    for name in name_exe:
        ...
        if not os.path.exists(name_exe[name]) and which(name_exe[name]) == None:
            ...
            write_error(con, "I cannot find the executable for " +
                        name + " at '" + name_exe[name] + "'")
            exit()
