...
def histogram(data, filekeyword, xlab="score", ylab="proportion"):
    ...
    cranstr += "bars <- read.table(\"" + \
        tablepath + "\", header=T, sep=\"\\t\")\n"
    ...
    cranstr += "barx = barplot(as.matrix(bars), beside=TRUE, ylim=range(0,1.0), names.arg=pointsets, xlab=\"" + \
        xlab + "\", ylab=\"" + ylab + "\");\n"
...
def scatter1(values_ii, values_jj, filekeyword, xlab="", ylab="", force_square=False):
    ...
    lim = max([maxx, maxy])
    if force_square:
        cranstr += ", xlim=range(0," + lim.__str__() + \
            "), ylim=range(0," + lim.__str__() + ")"
    ...
    cranstr += "text(" + ((lim - min(values_ii)) / 2).__str__() + ", " + \
        lim.__str__() + ", \"Prs R=%.2f" % rho + ", P=%.2f" % pvalue + "\");\n"

    """Spearman's non-linear non-parametric rank correlation."""
    (rho, pvalue) = scipystats.spearmanr(corr_valsa, corr_valsb)
    cranstr += "text(" + ((lim - min(values_ii)) / 2).__str__() + ", " + (0.92 *
                                                                          lim).__str__() + ", \"Spr R=%.2f" % rho + ", P=%.2f" % pvalue + "\");\n"
